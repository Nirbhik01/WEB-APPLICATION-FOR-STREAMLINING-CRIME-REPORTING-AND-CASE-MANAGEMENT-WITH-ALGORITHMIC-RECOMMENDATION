    ...
    if query:
        for search_term in query:
            ...
            # for admin
                ...
                matched_cases = list(Case.objects.filter(
                    case_title__icontains=search_term,is_registered=True
                ).values(
                    'case_title', 'upload_date', 'crime_description',
                    'case_id', 'reporter__user_profile_picture', 'reporter__user_name'
                )) 
...
def filter_cases(request):
    if request.method=="POST":  
        ...
        if request.session.get('user_type') == 'Investigator':
            ...
            case_list = Case.objects.filter(is_registered=True)
...
def display_cases_for_homepage(request):
    print("entered display cases for homepage")
    ...
    if user_type == 'Citizen':
        ...
        cases = Case.objects.filter(reporter=user, is_reporter_the_victim = True).exclude(
                                            status='Investigation_Termination').values('case_id','case_title','investigator__user_name','status')
        print(list(cases))
        ...
        cases = Case.objects.filter(investigator=user,is_registered=True).exclude(
                                            status='Investigation_Termination').values('case_id','case_title','reporter__user_name','status')
        return JsonResponse(list(cases), safe=False)
    
    elif user_type == 'Admin':
        # print("entered cases for admin")
        user=request.user
